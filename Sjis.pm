...
my $function_getc;        # getc    to getc    or Sjis::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Sjis::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Sjis::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Sjis::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Sjis::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Sjis::re_t=%s; local $Sjis::re_a=''; while($Sjis::re_t =~ %s){%s local $^W=0; local $Sjis::re_r=%s; %s$Sjis::re_t="$Sjis::re_a${1}$Sjis::re_r$'"; pos($Sjis::re_t)=length "$Sjis::re_a${1}$Sjis::re_r"; $Sjis::re_a=substr($Sjis::re_t,0,pos($Sjis::re_t)); } return $Sjis::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Sjis::re_r=eval $Sjis::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Sjis::re_t=%s; while($Sjis::re_t =~ %s){%s local $^W=0; local $Sjis::re_r=%s; %s$Sjis::re_t="$`$Sjis::re_r$'"; pos($Sjis::re_t)=length "$`$Sjis::re_r"; } return $Sjis::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Sjis::re_r=%s; %s"%s$Sjis::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Sjis::re_n=0; local $Sjis::re_a=''; while(%s =~ %s){%s local $^W=0; local $Sjis::re_r=%s; %s%s="$Sjis::re_a${1}$Sjis::re_r$'"; pos(%s)=length "$Sjis::re_a${1}$Sjis::re_r"; $Sjis::re_a=substr(%s,0,pos(%s)); $Sjis::re_n++} return %s$Sjis::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Sjis::re_n=0; while(%s =~ %s){%s local $^W=0; local $Sjis::re_r=%s; %s%s="$`$Sjis::re_r$'"; pos(%s)=length "$`$Sjis::re_r"; $Sjis::re_n++} return %s$Sjis::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Sjis::re_r=%s; %s%s="%s$Sjis::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Sjis::re_r=%s; %s%s="%s$Sjis::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Sjis::re_r=eval $Sjis::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=head1 Escaping Second Octet (Sjis.pm provides)

Sjis.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Sjis.pm provides)

Sjis.pm clusters multiple-octet character with quantifier, makes cluster from
...
Sjis.pm applies calling 'Esjis::ignorecase()' instead of /i modifier.
...
=item * Length by ShiftJIS Character
  ...
  the scalar value $string. If $string is omitted, it returns the Sjis::length of
  $_.
...
=item * Substr by ShiftJIS Character
  ...
  If Perl version 5.14 or later, you can use the Sjis::substr() function as an
...
=item * Rindex by ShiftJIS Character
  ...
  This function works just like Sjis::index except that it returns the character

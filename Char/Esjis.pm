...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Char::Esjis::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Char::Esjis::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Esjis::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
{
    ...
    @Char::Esjis::m_matched = (qr/(?{Char::Esjis::m_matched})/);
    @Char::Esjis::s_matched = (qr/(?{Char::Esjis::s_matched})/);
}
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Esjis::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
sub Char::Esjis::T(;*@) {
    ...
    local $_ = shift if @_;
    croak 'Too many arguments for -T (Char::Esjis::T)' if @_ and not wantarray;
    my $T = 1;
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Esjis::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Esjis::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Char::Esjis::split returns
...
=item Chop string
  ...
  The Char::Esjis::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Esjis::index($string,$substr,$position);
  $pos = Char::Esjis::index($string,$substr);
  ...
  the Char::Esjis::index function returns -1.

=item Reverse index string

  $pos = Char::Esjis::rindex($string,$substr,$position);
  $pos = Char::Esjis::rindex($string,$substr);

  Works just like Char::Esjis::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Char::Esjis::chr(65) is "A" in either ASCII or ShiftJIS, and
  Char::Esjis::chr(0x82a0) is a ShiftJIS HIRAGANA LETTER A. For the reverse of Char::Esjis::chr,
  use Char::Sjis::ord.

=item File test operator -X

  A file test operator is an unary operator that tests a pathname or a filehandle.
  If $string is omitted, it uses $_ by function Char::Esjis::r_.
  The following functions function when the pathname ends with chr(0x5C) on MSWin32.

  $test = Char::Esjis::r $string;
  $test = Char::Esjis::r_;
  ...
  a filehandle. Both Char::Esjis::T and Char::Esjis::B deosn't work when given the special
  ...
  $value = Char::Esjis::s $string;
  $value = Char::Esjis::s_;

  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Esjis::s(*), Char::Esjis::s_()   File has nonzero size (returns size)
...
=item Statistics about file
  ...
  ($dev,$ino,$mode,$nlink,$uid,$gid,$rdev,$size,
      $atime,$mtime,$ctime,$blksize,$blocks) = Char::Esjis::stat($file);
...
=item require file
  ...
  If the argument is a string, Char::Esjis::require loads and executes the Perl code found in
  the separate file whose name is given by the string. This is similar to performing a
  Char::Esjis::do on a file, except that Char::Esjis::require checks to see whether the library
  ...
  duplicate compilation.) Like its cousins Char::Esjis::do and use, Char::Esjis::require knows how
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Char::Esjis::binmode(FILEHANDLE, $disciplines);
  Char::Esjis::binmode(FILEHANDLE);
  Char::Esjis::binmode($filehandle, $disciplines);
  Char::Esjis::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Char::Esjis::open(FILEHANDLE, $mode, $expr);
  $rc = Char::Esjis::open(FILEHANDLE, $expr);
  $rc = Char::Esjis::open(FILEHANDLE);
  $rc = Char::Esjis::open(my $filehandle, $mode, $expr);
  $rc = Char::Esjis::open(my $filehandle, $expr);
  $rc = Char::Esjis::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Char::Esjis::open($fh, ">logfile")     # $fh is autovivified
  ...
  Char::Esjis::open my $fh, ">logfile" or die ...
  ...
  Char::Esjis::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Char::Esjis::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Char::Esjis::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Char::Esjis::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Char::Esjis::open(FH, "<:crlf:encoding(Char::Sjis)", $path) or die "can't open $path: $!";

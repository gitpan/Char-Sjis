...
######################################################################
#
# Char::Sjis - Source code filter to escape ShiftJIS script
#
# http://search.cpan.org/dist/Char-Char::Sjis/
#
...
my $qq_substr  = qr{(?: Char::Sjis::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $matched     = '';
my $s_matched   = '';
$matched        = q{$Char::Esjis::matched};
$s_matched      = q{ Char::Esjis::s_matched();};
...
my $function_ord;         # ord()   to ord()   or Char::Sjis::ord()
my $function_ord_;        # ord     to ord     or Char::Sjis::ord_
my $function_reverse;     # reverse to reverse or Char::Sjis::reverse
my $function_getc;        # getc    to getc    or Char::Sjis::getc
...
END
    ...
    print Char::Sjis::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Char::Esjis::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            Char::Esjis::unlink "$filename.e";
        }
        elsif (Char::Esjis::z("$filename.e")) {
            Char::Esjis::unlink "$filename.e";
        }
        else {
            my $e_mtime   = (Char::Esjis::stat("$filename.e"))[9];
            my $mtime     = (Char::Esjis::stat($filename))[9];
            my $__mtime__ = (Char::Esjis::stat(__FILE__))[9];
    ...
    if (not Char::Esjis::e("$filename.e")) {
        my $fh = gensym();

        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
        }
        else {
            Char::Esjis::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Sjis::escape_script($filename);
        print {$fh} $e_script;

        my $mode = (Char::Esjis::stat($filename))[2] & 0777;
    ...
    my $fh = gensym();
    Char::Esjis::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Sjis::escape_script {
    ...
    my $fh = gensym();
    Char::Esjis::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Esjis(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Esjis %s;\n", $Char::Sjis::VERSION); # require run-time routines version

        # use Char::Sjis version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Sjis \s* ([^\x81-\x9F\xE0-\xFC;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Sjis::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Sjis.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/Sjis.pm $version, but @{[__FILE__]} is version $Char::Sjis::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Esjis.pm $version, but \\\$Char::Esjis::VERSION is \$Char::Esjis::VERSION" if \$Char::Esjis::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Sjis::VERSION) {
                    die "Script $0 required Char/Sjis.pm $version, but @{[__FILE__]} is only version $Char::Sjis::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Sjis::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Sjis::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Sjis::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Sjis::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    # use Tk; --> use Char::SjisTk::Widget;
    my @tkmodule = ();
    for my $widget (qw(
        Button
        Canvas
        ...
        if (/ $widget /xms) {
            push @tkmodule, "        eval qq{ use Char::SjisTk::$widget; };";
        }
    }
    my $tkmodule = join "\n", @tkmodule;
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Esjis::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Esjis::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Esjis::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Esjis::MATCH()};
    }

# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Esjis::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Esjis::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Char::Esjis::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Esjis::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Esjis::glob';
    }
...
# functions of package Char::Esjis
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                    }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Esjis::chop';         }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';               }
    elsif (/\G \b Char::Sjis::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Sjis::index';         }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Esjis::index';        }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (/\G \b Char::Sjis::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Sjis::rindex';        }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Esjis::rindex';       }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::lc';           }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::lcfirst';      }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::uc';           }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::ucfirst';      }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::fc';           }
    ...
    # (and so on)
                                                         ...
                                                         \s* (\") ((?:$qq_char)+?)             (\") /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                         \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Esjis::filetest(qw($1),$2)"; }
    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) \( ((?:$qq_paren)*?) \) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Esjis::filetest(qw($1),$2)"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (?= [a-z]+) /oxgc)                                   { $slash = 'm//'; return "Char::Esjis::filetest qw($1),";    }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (\w+) /oxgc)                                         { $slash = 'm//'; return "Char::Esjis::filetest(qw($1),$2)"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Esjis::$1($2)";      }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* \( ((?:$qq_paren)*?) \) /oxgc)                          { $slash = 'm//'; return "Char::Esjis::$1($2)";      }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= \s+ [a-z]+) /oxgc)                                      { $slash = 'm//'; return "Char::Esjis::$1";          }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ (\w+) /oxgc)                                            { $slash = 'm//'; return "Char::Esjis::$1(::"."$2)"; }
    elsif (/\G -(t)                            \s+ (\w+) /oxgc)                                            { $slash = 'm//'; return "-t $2";              }
    elsif (/\G \b lstat         (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::lstat';             }
    elsif (/\G \b stat          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::stat';              }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Esjis::glob';              }
    elsif (/\G \b lc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Esjis::lc_';               }
    elsif (/\G \b lcfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Esjis::lcfirst_';          }
    elsif (/\G \b uc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Esjis::uc_';               }
    elsif (/\G \b ucfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Esjis::ucfirst_';          }
    elsif (/\G \b fc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Esjis::fc_';               }
    elsif (/\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                           \b    (?! \s* => )                        /oxgc) { $slash = 'm//'; return "Char::Esjis::filetest_(qw($1))"; }
    elsif (/\G    -([rwxoRWXOezsfdlpSbcugkTBMAC])
                           \b    (?! \s* => )                        /oxgc) { $slash = 'm//'; return "Char::Esjis::${1}_";             }
    elsif (/\G \b lstat \b      (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Esjis::lstat_';            }
    elsif (/\G \b stat \b       (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Esjis::stat_';             }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Esjis::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Esjis::glob_';             }
    elsif (/\G \b reverse \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return $function_reverse;          }
    elsif (/\G \b getc \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return $function_getc;             }
    elsif (/\G \b opendir (\s* \( \s*) (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Char::Esjis::opendir$1*";        }
    elsif (/\G \b opendir (\s+)        (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Char::Esjis::opendir$1*";        }
    elsif (/\G \b unlink \b     (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Esjis::unlink';            }

# chdir
    elsif (/\G \b (chdir) \b    (?! \s* => ) /oxgc) {
        $slash = 'm//';

        my $e = 'Char::Esjis::chdir';
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Esjis::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Esjis::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Esjis::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Esjis::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Esjis::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Esjis::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Esjis::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Esjis::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Esjis::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Esjis::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Esjis::split' . $e . qq     {" "};  }
...
# do
    ...
    elsif (/\G \b do (?= \s+ (?: q|qq|qx) \b)        /oxmsgc)                                  { return 'Char::Esjis::do';         }
    elsif (/\G \b do (?= \s+ \w+)                    /oxmsgc)                                  { return 'do';                }
    elsif (/\G \b do (?= \s* \$ \w+ (?: ::\w+)* \( ) /oxmsgc)                                  { return 'do';                }
    elsif (/\G \b do \b                              /oxmsgc)                                  { return 'Char::Esjis::do';         }
...
# require else
    elsif (/\G \b require                                   \s* ; /oxmsgc)                     { return 'Char::Esjis::require;';   }
    elsif (/\G \b require \b                                      /oxmsgc)                     { return 'Char::Esjis::require';    }
...
# functions of package Char::Esjis
        ...
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Esjis::chop';    $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::Sjis::index \b                               /oxgc) { $e_string .= 'Char::Sjis::index';    $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Esjis::index';   $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::Sjis::rindex \b                              /oxgc) { $e_string .= 'Char::Sjis::rindex';   $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Esjis::rindex';  $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::lc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::uc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::fc';      $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\") ((?:$qq_char)+?)             (\") /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Char::Esjis::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                          { $e_string .= "Char::Esjis::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) \( ((?:$qq_paren)*?) \) /oxgc)
                                                                                                                          { $e_string .= "Char::Esjis::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (?= [a-z]+) /oxgc)                                   { $e_string .= "Char::Esjis::filetest qw($1),";    $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (\w+) /oxgc)                                         { $e_string .= "Char::Esjis::filetest(qw($1),$2)"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Char::Esjis::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                          { $e_string .= "Char::Esjis::$1($2)";      $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* \( ((?:$qq_paren)*?) \) /oxgc)                          { $e_string .= "Char::Esjis::$1($2)";      $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= \s+ [a-z]+) /oxgc)                                      { $e_string .= "Char::Esjis::$1";          $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ (\w+) /oxgc)                                            { $e_string .= "Char::Esjis::$1(::"."$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G -(t)                            \s+ (\w+) /oxgc)                                            { $e_string .= "-t $2";              $slash = 'm//'; }
        elsif ($string =~ /\G \b lstat         (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::lstat';             $slash = 'm//'; }
        elsif ($string =~ /\G \b stat          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::stat';              $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Esjis::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Esjis::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Esjis::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Esjis::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Esjis::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Esjis::fc_';               $slash = 'm//'; }
        elsif ($string =~ /\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                 \b                 /oxgc) { $e_string .= "Char::Esjis::filetest_(qw($1))"; $slash = 'm//'; }
        elsif ($string =~ /\G    -([rwxoRWXOezsfdlpSbcugkTBMAC]) \b                 /oxgc) { $e_string .= "Char::Esjis::${1}_";             $slash = 'm//'; }
        elsif ($string =~ /\G \b lstat \b                                           /oxgc) { $e_string .= 'Char::Esjis::lstat_';            $slash = 'm//'; }
        elsif ($string =~ /\G \b stat \b                                            /oxgc) { $e_string .= 'Char::Esjis::stat_';             $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Esjis::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Esjis::glob_';             $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b opendir (\s* \( \s*) (?=[A-Za-z_])                 /oxgc) { $e_string .= "Char::Esjis::opendir$1*";        $slash = 'm//'; }
        elsif ($string =~ /\G \b opendir (\s+)        (?=[A-Za-z_])                 /oxgc) { $e_string .= "Char::Esjis::opendir$1*";        $slash = 'm//'; }
        elsif ($string =~ /\G \b unlink \b                                          /oxgc) { $e_string .= 'Char::Esjis::unlink';            $slash = 'm//'; }

# chdir
        elsif ($string =~ /\G \b (chdir) \b (?! \s* => ) /oxgc) {
            $slash = 'm//';

            $e_string .= 'Char::Esjis::chdir';
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Esjis::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Esjis::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Esjis::dot_s}';
        }
        else {
            return '${Char::Esjis::dot}';
        }
    }
    else {
        return Char::Esjis::classic_character_class($char);
    }
}
...
sub e_capture {
    ...
    return join '', '${Char::Esjis::capture(', $_[0], ')}';
    return join '', '$',                 $_[0];
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Esjis::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Esjis::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Esjis::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Esjis::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Esjis::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Esjis::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Esjis::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Esjis::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Esjis::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Esjis::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Esjis::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Esjis::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Esjis::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Esjis::uc($char[$i]) ne Char::Esjis::fc($char[$i]))) {
            if (CORE::length(Char::Esjis::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Esjis::uc($char[$i])       . Char::Esjis::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Esjis::uc($char[$i]) . '|' . Char::Esjis::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Esjis::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Esjis::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Esjis::ignorecase(Char::Esjis::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Esjis::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Esjis::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Esjis::ignorecase(Char::Esjis::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Esjis::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Esjis::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Esjis::ignorecase(Char::Esjis::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Esjis::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::Sjis::re_t=%s; local $Char::Sjis::re_a=''; while($Char::Sjis::re_t =~ %s){%s local $^W=0; local $Char::Sjis::re_r=%s; %s$Char::Sjis::re_t="$Char::Sjis::re_a${1}$Char::Sjis::re_r$'"; pos($Char::Sjis::re_t)=length "$Char::Sjis::re_a${1}$Char::Sjis::re_r"; $Char::Sjis::re_a=substr($Char::Sjis::re_t,0,pos($Char::Sjis::re_t)); } return $Char::Sjis::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Sjis::re_r=eval $Char::Sjis::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Sjis::re_t=%s; while($Char::Sjis::re_t =~ %s){%s local $^W=0; local $Char::Sjis::re_r=%s; %s$Char::Sjis::re_t="$`$Char::Sjis::re_r$'"; pos($Char::Sjis::re_t)=length "$`$Char::Sjis::re_r"; } return $Char::Sjis::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Sjis::re_r=%s; %s"%s$Char::Sjis::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::Sjis::re_n=0; local $Char::Sjis::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::Sjis::re_r=%s; %s%s="$Char::Sjis::re_a${1}$Char::Sjis::re_r$'"; pos(%s)=length "$Char::Sjis::re_a${1}$Char::Sjis::re_r"; $Char::Sjis::re_a=substr(%s,0,pos(%s)); $Char::Sjis::re_n++} return %s$Char::Sjis::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Sjis::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Sjis::re_r=%s; %s%s="$`$Char::Sjis::re_r$'"; pos(%s)=length "$`$Char::Sjis::re_r"; $Char::Sjis::re_n++} return %s$Char::Sjis::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Sjis::re_r=%s; %s%s="%s$Char::Sjis::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Sjis::re_r=%s; %s%s="%s$Char::Sjis::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Sjis::re_r=eval $Char::Sjis::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_chdir_q {
    ...
    if ($^W) {
        if (Char::Esjis::_MSWin32_5Cended_path($string)) {
            if ($] !~ /^5\.005/) {
                warn <<END;
...
sub e_split {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # ${ foo }
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Esjis::ignorecase(' . $1 . ')]}';
            }
        }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Esjis::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Esjis::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Esjis::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::Sjis \s* ([^\x81-\x9F\xE0-\xFC;]*) ; \s* \n? $/oxms) {
                ...
                return qq[BEGIN { Char::Esjis::require '$expr'; $module->import() if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no_noparam {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Esjis::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::Sjis \s* ([^\x81-\x9F\xE0-\xFC;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Esjis::require '$expr'; $module->unimport() if $module->can('unimport'); }];
            }
            last;
        }
    }
...
sub e_use {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Esjis::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::Sjis \s* ([^\x81-\x9F\xE0-\xFC;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Esjis::require '$expr'; $module->import($list) if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Esjis::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::Sjis \s* ([^\x81-\x9F\xE0-\xFC;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Esjis::require '$expr'; $module->unimport($list) if $module->can('unimport'); }];
            }
            last;
        }
    }
...
=item * You'll have to download Char/Sjis.pm and Char/Esjis.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Sjis;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Sjis;
  use Char::Sjis ver.sion;             --- require minimum version
  use Char::Sjis ver.sion.0;           --- expects version (match or die)
  use Char::Sjis qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::Sjis ver.sion qw(ord reverse getc);
  use Char::Sjis ver.sion.0 qw(ord reverse getc);

  # "no Char::Sjis;" not supported

  or

  $ perl Char/Sjis.pm ShiftJIS_script.pl > Escaped_script.pl.e
...
Char::Sjis software is "middleware" between perl interpreter and your Perl script
written in ShiftJIS.
...
By "use Char::Sjis;", it automatically interpret your script as ShiftJIS. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Sjis.pm, Char/Esjis.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Esjis.pm
works well.
...
=head1 Software Composition
   ...
   Char/Sjis.pm               --- source code filter to escape ShiftJIS
   Char/Esjis.pm              --- run-time routines for Char/Sjis.pm
...
You need write 'use Char::Sjis;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Sjis;
  ---------------------------------

=head1 Escaping Multiple-Octet Code (Char/Sjis.pm provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/Sjis.pm provides)

Char/Sjis.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Esjis::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Escaping Second Octet (Char/Sjis.pm provides)

Char/Sjis.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char/Sjis.pm provides)

Char/Sjis.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Esjis::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Esjis::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Esjis::ignorecase()' (Char/Sjis.pm provides)

Char/Sjis.pm applies calling 'Char::Esjis::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Esjis::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Esjis::matched/
  m/regexp/               m/ditto$Char::Esjis::matched/
  ?regexp?                m?ditto$Char::Esjis::matched?
  m?regexp?               m?ditto$Char::Esjis::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Esjis::matched/) ?
  s/regexp/replacement/   eval{ Char::Esjis::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Esjis::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Esjis::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Esjis::split(qr/regexp/)
  split(m/regexp/)        Char::Esjis::split(qr/regexp/)
  split(qr/regexp/)       Char::Esjis::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Esjis::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Esjis::matched/
  m/regexp/b              m/(?:regexp)$Char::Esjis::matched/
  ?regexp?b               m?regexp$Char::Esjis::matched?
  m?regexp?b              m?regexp$Char::Esjis::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Esjis::matched/) ?
  s/regexp/replacement/b  eval{ Char::Esjis::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Esjis::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Esjis.pm provides)
...
Definitions in Char/Esjis.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Char::Esjis::anchor}         qr{\G(?:[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFC])*?}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Sjis.pm provides)

Char/Sjis.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Sjis.pm and Char/Esjis.pm provide)

Insert 'Char::Esjis::' at head of function name. Char/Esjis.pm provides your script Char::Esjis::*
functions.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Esjis::split     Character
  tr///       Char::Esjis::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Esjis::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Esjis::chop      Character
  index       Char::Esjis::index     Character
  rindex      Char::Esjis::rindex    Character
  lc          Char::Esjis::lc        Character
  lcfirst     Char::Esjis::lcfirst   Character
  uc          Char::Esjis::uc        Character
  ucfirst     Char::Esjis::ucfirst   Character
  fc          Char::Esjis::fc        Character
  chr         Char::Esjis::chr       Character
  glob        Char::Esjis::glob      Character
  lstat       Char::Esjis::lstat     Character
  opendir     Char::Esjis::opendir   Character
  stat        Char::Esjis::stat      Character
  unlink      Char::Esjis::unlink    Character
  chdir       Char::Esjis::chdir     Character
  do          Char::Esjis::do        Character
  require     Char::Esjis::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Char::Esjis::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Char::Esjis::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Char::Esjis::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Char::Esjis::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Char::Esjis::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Char::Esjis::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (Char/Sjis.pm and Char/Esjis.pm provide)

Insert 'Char::Esjis::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Char::Esjis::r   File or directory is readable by this (effective) user or group
  -w       Char::Esjis::w   File or directory is writable by this (effective) user or group
  -e       Char::Esjis::e   File or directory name exists
  -x       Char::Esjis::x   File or directory is executable by this (effective) user or group
  -z       Char::Esjis::z   File exists and has zero size (always false for directories)
  -f       Char::Esjis::f   Entry is a plain file
  -d       Char::Esjis::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Char::Esjis::T   File looks like a "text" file
  -B       Char::Esjis::B   File looks like a "binary" file
  -M       Char::Esjis::M   Modification age (measured in days)
  -A       Char::Esjis::A   Access age (measured in days)
  -C       Char::Esjis::C   Inode-modification age (measured in days)
  -s       Char::Esjis::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Char::Esjis::R   File or directory is readable by this real user or group
  -W       Char::Esjis::W   File or directory is writable by this real user or group
  -X       Char::Esjis::X   File or directory is executable by this real user or group
  -l       Char::Esjis::l   Entry is a symbolic link
  -S       Char::Esjis::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Char::Esjis::o   File or directory is owned by this (effective) user
  -O       Char::Esjis::O   File or directory is owned by this real user
  -p       Char::Esjis::p   Entry is a named pipe (a "fifo")
  -b       Char::Esjis::b   Entry is a block-special file (like a mountable disk)
  -c       Char::Esjis::c   Entry is a character-special file (like an I/O device)
  -u       Char::Esjis::u   File or directory is setuid
  -g       Char::Esjis::g   File or directory is setgid
  -k       Char::Esjis::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'Char::Sjis::' at head of function name when you want character-
oriented function. See 'Character-Oriented Functions'.
  ...
  --------------------------------------------------------
  ord        Char::Sjis::ord
  reverse    Char::Sjis::reverse
  getc       Char::Sjis::getc
  length     Char::Sjis::length
  substr     Char::Sjis::substr
  index      Char::Sjis::index          See 'About Indexes'
  rindex     Char::Sjis::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Esjis::index)
  Char::Sjis::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Esjis::rindex)
  Char::Sjis::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = Char::Sjis::ord($string);

  This function returns the numeric value (ASCII or ShiftJIS character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use Char::Sjis qw(ord);", ord of your script will be rewritten in
  Char::Sjis::ord. Char::Sjis::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::Sjis::reverse(@list);
  $reverse = Char::Sjis::reverse(@list);
  ...
  If you import reverse "use Char::Sjis qw(reverse);", reverse of your script will be
  rewritten in Char::Sjis::reverse. Char::Sjis::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::Sjis::getc(FILEHANDLE);
  $getc = Char::Sjis::getc($filehandle);
  $getc = Char::Sjis::getc;
  ...
  If you import getc "use Char::Sjis qw(getc);", getc of your script will be rewritten in
  Char::Sjis::getc. Char::Sjis::getc is not compatible with getc of JPerl.

=item * Length by ShiftJIS Character

  $length = Char::Sjis::length($string);
  $length = Char::Sjis::length();

  This function returns the length in characters (programmer-visible characters) of
  the scalar value $string. If $string is omitted, it returns the Char::Sjis::length of
  $_.

  Do not try to use Char::Sjis::length to find the size of an array or hash. Use scalar
...
=item * Substr by ShiftJIS Character
  ...
  $substr = Char::Sjis::substr($string,$offset,$length,$replacement);
  $substr = Char::Sjis::substr($string,$offset,$length);
  $substr = Char::Sjis::substr($string,$offset);
  ...
  my $color  = Char::Sjis::substr $s, 4, 5;      # black
  my $middle = Char::Sjis::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::Sjis::substr $s, 14;        # climbed the green tree
  my $tail   = Char::Sjis::substr $s, -4;        # tree
  my $z      = Char::Sjis::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::Sjis::substr() function as an
  ...
  end of the string, Char::Sjis::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::Sjis::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::Sjis::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::Sjis::substr $name, 7;      # returns undef, with warning
  Char::Sjis::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::Sjis::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::Sjis::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::Sjis::substr() acts
...
=item * Index by ShiftJIS Character
  ...
  $index = Char::Sjis::index($string,$substring,$offset);
  $index = Char::Sjis::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::Sjis::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by ShiftJIS Character
  ...
  $rindex = Char::Sjis::rindex($string,$substring,$offset);
  $rindex = Char::Sjis::rindex($string,$substring);

  This function works just like Char::Sjis::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::Sjis::index).
  ...
  $pos = Char::Sjis::length($string);
  while (($pos = Char::Sjis::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Esjis::glob() function. See Char::Esjis::glob of
  Char/Esjis.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::Sjis qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::Sjis qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Functions (Char/Sjis.pm provides)

Char/Sjis.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Esjis.pm provides)

Char/Esjis.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Sjis' }" at head.
Store the standard module modified for Char::Sjis software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Sjis and change
'use utf8;' to 'use Char::Sjis;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Esjis.pm provides these
  functions.
...
=head1 MacJPerl on The MacOS
 ...
 The following software is necessary to execute Char::Sjis software.
 1. MacPerl module
 2. Mac::Files module
 3. ToolServer
...
=item * Char::Sjis::substr as Lvalue

Char::Sjis::substr differs from CORE::substr, and cannot be used as a lvalue.
To change part of a string, you can use the optional fourth argument which is the
replacement string.

Char::Sjis::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Esjis::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Esjis::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Esjis::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Esjis::MATCH()       $1
  $MATCH          Char::Esjis::MATCH()       $1
  ${^MATCH}       Char::Esjis::MATCH()       $1
  $'              Char::Esjis::POSTMATCH()   $'
  $POSTMATCH      Char::Esjis::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Esjis::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Char::Esjis::*(),
Char::Esjis::lstat(), and Char::Esjis::stat() on Microsoft Windows open the file for the path
...
Char::Sjis software remains one language and one interpreter.

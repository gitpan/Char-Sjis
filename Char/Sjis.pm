...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Esjis::function_ord     = 'Char::Sjis::ord'     if /\A ord \z/oxms;
                    $Char::Esjis::function_ord_    = 'Char::Sjis::ord_'    if /\A ord \z/oxms;
                    $Char::Esjis::function_reverse = 'Char::Sjis::reverse' if /\A reverse \z/oxms;
                    $Char::Esjis::function_getc    = 'Char::Sjis::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
    ...
    for my $widget (qw(
        ...
        if (/ $widget /xms) {
            push @tkmodule, "        CORE::eval qq{ use Char::SjisTk::$widget; };";
        }
    }
    my $tkmodule = join "\n", @tkmodule;
...
=head1 Escaping Multiple-Octet Code (Char::Sjis software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char::Sjis software provides)

Char::Sjis software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Escaping Second Octet (Char::Sjis software provides)

Char::Sjis software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (Char::Sjis software provides)

Char::Sjis software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Esjis::ignorecase()' (Char::Sjis software provides)

Char::Sjis software applies calling 'Char::Esjis::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Esjis::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Esjis::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Esjis::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Esjis::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::Sjis software provides)

Char::Sjis software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::Sjis software provide)
...
=head1 Escaping File Test Operators (Char::Sjis software provide)
...
=item * Eval ShiftJIS Script
  ...
  $eval = Char::Sjis::eval { block };
  $eval = Char::Sjis::eval $expr;
  $eval = Char::Sjis::eval;

  The Char::Sjis::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::Sjis::eval { block }
  and Char::Sjis::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::Sjis::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::Sjis::eval, the value returned from an Char::Sjis::eval is
  ...
  Char::Sjis::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::Sjis::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::Sjis::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::Sjis::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::Sjis::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::Sjis::eval cannot trap syntax errors.

  The Char::Sjis::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::Sjis::eval is complete, as do any subroutine or format definitions.
  The code of the Char::Sjis::eval is treated as a block, so any locally scoped
  variables declared within the Char::Sjis::eval last only until the Char::Sjis::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::Sjis::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::Sjis::eval { block }    eval { block }
  Char::Sjis::eval $expr        eval Char::Sjis::escape $expr
  Char::Sjis::eval              eval Char::Sjis::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::Sjis::eval does not exist. If it exists,
  you will troubled, when Char::Sjis::eval has a parameter that is single quoted
  string included my variables. Char::Sjis::escape is a subroutine that makes Perl
  ...
  Since Char::Sjis::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::Sjis::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::Sjis::eval $expr and Char::Sjis::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::Sjis::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::Sjis::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::Sjis::eval '$answer =';   # sets $@
  ...
  Using the Char::Sjis::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::Sjis::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::Sjis::eval { die "foo lives here" };
  ...
  With an Char::Sjis::eval, you should be especially careful to remember what's being
  looked at when:

      Char::Sjis::eval $x;        # CASE 1
      Char::Sjis::eval "$x";      # CASE 2

      Char::Sjis::eval '$x';      # CASE 3
      Char::Sjis::eval { $x };    # CASE 4

      Char::Sjis::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Char::Sjis::eval { test_repugnancy() };
  ...
  The block of Char::Sjis::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::Sjis software provides)

Char::Sjis software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::Sjis software and

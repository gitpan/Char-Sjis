...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Esjis::uc($char[$i]) ne Char::Esjis::lc($char[$i]))) {
            $char[$i] = '[' . Char::Esjis::uc($char[$i]) . Char::Esjis::lc($char[$i]) . ']';
        }
...
sub e_require {
    ...
    return qq<Char::Esjis::require '$expr';>;
}
...
http://search.cpan.org/dist/Char-Char::Sjis/Char::Sjis/JA.pm
...
Definitions in Char/Esjis.pm.
  ...
  @{Char::Esjis::eV}             qr{(?:[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFC\x0C\x0A\x0D])}
...
=item * Length By ShiftJIS Character
  ...
  $string is omitted, it returns the Char::Sjis::length of $_.
...
=item * Substr By ShiftJIS Character
  ...
  area past the end of the string, Char::Sjis::substr raises an exception.
...
=item * Rindex By ShiftJIS Character
  ...
  $rindex = Char::Sjis::rindex($string,$substring,$position);
  $rindex = Char::Sjis::rindex($string,$substring);

  This function works just like Char::Sjis::index except that it returns the position of
